...
static void
on_ssrc_active (GObject * session, GObject * source, GstRTSPMedia * UNUSED(media))
{
  GstStructure *stats;

  GST_INFO ("source %p in session %p is active", (void*)source, (void*)session);
...
static void
on_sender_ssrc_active (GObject * session, GObject * source, GstRTSPMedia * UNUSED(media))
{
  GstStructure *stats;
...
media_prepared_cb (GstRTSPMedia * media)
{
  ...
  GST_INFO ("media %p is prepared and has %u streams", (void*)media, n_streams);
